...
const Promotion = () => {
  ...
  const [dishes, setDishes] = useState([])
    const fetchEvent = async(uuid) => {
        ...
        await axios.post(`https://order-automation-debug-server.onrender.com/api/get-dish-by-event/${uuid}`)
        .then(res => {
            setDishes(res.data)
        })
    }
            ...
            <div className='w-full absolute top-80 rounded-t-3xl h-1/2 bg-[#FACD5D]'>
                ...
                <div className='w-full h-full absolute bg-[#F7F7F7] rounded-t-3xl px-6'>
                    ...
                    <div className='flex flex-col w-full space-y-8'>
                        {dishes.map((dish) => (
                            <div onClick={() => navigateFoodDetails(dish.uuid)} className='w-full h-32 flex bg-white rounded-xl'>
